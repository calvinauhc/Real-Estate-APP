...
if view_mode == "Admin Dashboard Backend":
    ...
    if admin_password == "admin123":
        ...
        df = pd.read_csv(data_layer.AGENTS_CSV, names=["name", "license", "status"]) if os.path.getsize(data_layer.AGENTS_CSV) > 0 else pd.DataFrame(columns=["name", "license", "status"])
        pending_agents = df[df["status"] == "pending"]
        
        st.subheader("📥 Pending Approvals")
        if pending_agents.empty: st.info("No agents waiting.")
        else:
            for index, row in pending_agents.iterrows():
                col1, col2, col3 = st.columns([3, 1, 1])
                col1.write(f"👤 **{row['name']}** ({row['license']})")
                if col2.button("✅ Approve", key=f"app_{row['license']}"):
                    df.at[index, "status"] = "active"
                    df.to_csv(data_layer.AGENTS_CSV, index=False, header=False)
                    st.rerun()
                if col3.button("❌ Reject", key=f"rej_{row['license']}"):
                    df.at[index, "status"] = "rejected"
                    df.to_csv(data_layer.AGENTS_CSV, index=False, header=False)
                    st.rerun()
    else: st.warning("Please enter admin credentials.")
...
# ==========================================
    ...
    with tab1:
        ...
        with col_right:
            ...
            target_property_type = st.selectbox("Target Property Category", ["Private Condominium", "HDB Resale Flat"])
            ...
            if space_violation: 
                ...
                try:
                    pk_saved = data_layer.save_client_profile(
                        c_name, c_phone, current_property_value, outstanding_loan,
                        total_accrued_cpf_to_return, p_high, available_cash_savings, total_static_cpf
                    )
